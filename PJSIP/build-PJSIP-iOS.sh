...
find ./ -name '*.a' | grep -v '^\.\/\/lib' | xargs -J ^ mv ^ ./lib/armv7/

export ARCH='-arch armv7s'
make realclean
./configure-iphone --with-ssl="${OPENSSL_DIR}"
make realclean depend all
find ./ -name '*.a' | grep -v '^\.\/\/lib' | xargs -J ^ mv ^ ./lib/armv7s/

export ARCH='-arch arm64'
make realclean
./configure-iphone --with-ssl="${OPENSSL_DIR}"
make realclean depend all
find ./ -name '*.a' | grep -v '^\.\/\/lib' | xargs -J ^ mv ^ ./lib/arm64/
...
./configure-iphone --with-ssl="${OPENSSL_DIR}"
make realclean depend all
find ./ -name '*.a' | grep -v '^\.\/\/lib' | xargs -J ^ mv ^ ./lib/i386/

export ARCH='-arch x86_64'
make realclean
./configure-iphone --with-ssl="${OPENSSL_DIR}"
make realclean depend all
find ./ -name '*.a' | grep -v '^\.\/\/lib' | xargs -J ^ mv ^ ./lib/x86_64/

cd ./lib/arm64
ls -1| sed -e 's/^\(\(.*\)-arm-apple-darwin9\.a\)/\1 \2.a/' | xargs -n2 mv
cd ../armv7
ls -1| sed -e 's/^\(\(.*\)-arm-apple-darwin9\.a\)/\1 \2.a/' | xargs -n2 mv
cd ../armv7s
ls -1| sed -e 's/^\(\(.*\)-arm-apple-darwin9\.a\)/\1 \2.a/' | xargs -n2 mv
